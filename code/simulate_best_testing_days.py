...
def event(t, Y, M, min_val=2e6, target="peak", *args):
    if t < 1.0:
        return 1

    dy = M.odes(t=t, prev_pop=Y, flag_track_incidence=True)
    I_total = 0
    for cc in M.CC:
        if "nc" in cc.name:
            continue
...
#         model_val = Y[M.CC["Inc"]]

#         return model_val - min_val
...
def find_best_testing(pars, target,
    ...
    for idx, x in enumerate(combinations):
        ...
        event_tmp = lambda t, Y, *args: event(t, Y, M=M,
                                              min_val=target_min, target=target)
        event_tmp.terminal = 2
        ...
        c_peak = get_metric(M, target=target)

        if c_peak < target_min:
...
def _evaluate_testing_idx(args):
    ...
    try:
        ...
        return idx, ans[2], ans[0]
    except Exception:
        return idx, -1, -1  # fallback if model fails
...
def create_testing_region_data(input_params,
    ...
    if generate_data_flag:
        ...
        testing_val_regions = [None] * len(iter_vals)

        with ProcessPoolExecutor(max_workers=cpu_count()-1) as executor:
            for idx, value1, value2 in executor.map(_evaluate_testing_idx, args_list):
                testing_idx_regions[idx] = value1
                testing_val_regions[idx] = value2

        testing_idx_regions = np.array(
            testing_idx_regions).reshape(grid_vals[0].shape)
        testing_val_regions = np.array(
            testing_val_regions).reshape(grid_vals[0].shape)

        dat = {
            "grid_vals": grid_vals,
            "testing_idx_regions": testing_idx_regions,
            "testing_val_regions": testing_val_regions,
            "r0_d": r0_d,
            "r0_b": r0_b
        }
...
if __name__ == "__main__":
    ...
    for t in ["peak", "final_size"]:
        ...
        plt.figure()
        plt.title(f"Target ID for min: {t}")
        plt.tight_layout()
        ...
        plt.figure()
        plt.title(f"Target value for min: {t}")
        plt.tight_layout()
        im = plt.contourf(ans["grid_vals"][1], ans["grid_vals"][0],
                          ans["testing_val_regions"],
                          # levels=lvls,
                          # cmap=cmap
                          )
        plt.contour(ans["grid_vals"][1], ans["grid_vals"][0],
